...
- Processing of entire directories of PDFs
...
## Output
If processing is successful, a new PDF with embedded metadata is created in an auto-generated Success directory on the desktop. The file is renamed to match the eBook's title, while the original remains unchanged.
...
3. **Resolve Conflicts**  
   Applies majority voting and normalization to select the most reliable metadata. Prompts the user if results are ambiguous or conflicting.
...
Processing a PDF typically takes around 14 seconds, depending on connection speed, response time from metadata sources, and computer performance.


## Using the Source Code

PDF Book Tagger is fully functional as a PowerShell script. You can download:
- *pdf-book-tagger.ps1*
- *lib/SchedulesExtractor.jar*
from repository and use the script directly without any compilation.
