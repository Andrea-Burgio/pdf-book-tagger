...
function IsLcc([string]$targetString) {
    ...
    if ([string]::IsNullOrWhiteSpace($targetString)) { return $false }
...
<#
  Retrieves the full line from an LCC schedule file corresponding to a given LCC code.
  ...
  - Searches the file for a line whose code matches the given LCC.
  - Also handles collapsed "A-Z" ranges (e.g., "QA9.V38" will match "QA9.A5-Z").
  Throws an error if the appropriate file is not found.
#>
...
function GetCanonicalIdentifier([string]$word) {
    ...
    $technicalIdentifiers = @( # Acronyms, initialisms, abbreviations, and identifiers to preserve in original casing
        '.NET', '2FA', '5G',

        'ABAC', 'ACK', 'ADC', 'ADSL', 'AGI', 'AGP', 'AI', 'AKS', 'API', 'APN', 'ARM', 'AR',
        'AR/VR', 'ASIC', 'ASP.NET', 'ASSP', 'ASP', 'AST',

        'BERT', 'BI', 'BGP', 'BJT', 'BIOS', 'BDD', 'BASE', 'BLOB', 'BOM', 'BSD',

        'C#', 'C++', 'C/C++', 'CAD', 'CAN', 'CAP', 'CA', 'CDN', 'CFG', 'CI', 'CI/CD', 'CISC', 'CLI', 'CMS', 'CMOS',
        'CNN', 'CSV', 'CSS', 'CRUD', 'CVE', 'CVSS',

        'DAC', 'DDoS', 'DDR', 'DB', 'DBMS', 'DDL', 'DLP', 'DML', 'DNS', 'DoS', 'DRY', 'DSL', 'DPI', 'D3.js',


        'EISA', 'EEPROM', 'EDGE', 'EKS', 'EMF', 'ERP', 'ETL', 'EtherCAT', 'EULA',

        'FPGA', 'FaaS', 'FOSS', 'FTP',

        'GAN', 'GKE', 'GNU', 'GPU', 'GPRS', 'GPS', 'GUI', 'GPT', 'HTML',

        'HTTP', 'HTTP/2', 'HTTPS', 'HDD',

        'iBT', 'IC', 'ICMP', 'IDE', 'IDS', 'IE', 'IGMP', 'IMAP', 'IMU', 'IoT', 'I/O', 'IP', 'IPS', 'ISA', 'ISP',
        'IaC', 'I2C', 'I2S'

        'JPG', 'JPEG', 'JTAG', 'JSON', 'JSONB', 'JVM', 'JWT',

        'K8s', 'KISS', 'KNN',

        'LAN', 'LAMP', 'LCD', 'LDAP', 'LED', 'LIN', 'LiDAR', 'LPDDR', 'LSP', 'LTE',

        'MAC', 'MAN', 'MCA', 'MITM', 'MIPS', 'ML', 'MMU', 'MOSFET', 'MVC', 'MVVM', 'MPLS', 'MR', 'MFA',

        'NAND', 'NAT', 'NFC', 'NFV', 'NLP', 'NOR', 'NoSQL', 'NVMe',

        'OAuth', 'OCR', 'OLAP', 'OLTP', 'OLED', 'ORM', 'OS', 'OSI', 'OTP', 'OOP', 'OSS', 'OTP',

        'PaaS', 'PCB', 'PCIe', 'PDF', 'PET', 'PKI', 'PNG', 'PnP', 'POP3', 'POSIX', 'PoE', 'PPT', 'PSU', 'PSTN',

        'QR', 'QUIC', 'SRTP'

        'RDBMS', 'RAID', 'RAM', 'RBAC', 'REPL', 'REST', 'RISC', 'RL', 'ROM', 'ROS', 'RPC', 'RTCP', 'RTK', 'RTOS',
        'RTP', 'RxJS',

        'SaaS', 'SAML', 'SATA', 'SDK', 'SDN', 'SDR', 'SEO', 'SFTP', 'SHA', 'SIMM', 'SIEM', 'SMPS', 'SMTP',
        'SNMP', 'SOC', 'SoC', 'SODIMM', 'SPI', 'SQL', 'SQLi', 'SR', 'SSD', 'SSH', 'SSL', 'SSO',
        'SVG', 'SVGA', 'SVM',

        'TDD', 'TCP', 'TFT', 'TLS', 'TPM', 'TOEFL', 'TTP', 'TUI',

        'UART', 'UDP', 'UEFI', 'UI', 'UML', 'URI', 'URL', 'USB-C',

        'VCS', 'VDI', 'VGA', 'VLAN', 'VM', 'VPN', 'VoIP',

        'WAN', 'WAMP', 'WPA', 'WYSIWYG', 'Wi-Fi', 'WWW', 'WebRTC', 'WebSockets',

        'XAMPP', 'XML', 'XSS', 'XR'
    )
    $romanNumerals = @( # Roman numerals commonly used in titles or edition markers (e.g., "Volume III")
        "II", "III", "IV", "V",
        "VI", "VII", "VIII", "IX", "X",
        "XI", "XII", "XIII", "XIV", "XV",
    ...
    $standardIdentifiers = $technicalIdentifiers + $romanNumerals
...
function GetData_Yale([string]$targetBook){
    ...
    function BuildBookData_Yale($bookPageContent) {

        # Handle title
        # Title = Group 2. Group 1 is used to match the <h> element
        $titlePattern = '<div class="title-main">\s*<(h\d)>([^<]*)</\1>'
        $titleMatches = [regex]::Matches($bookPageContent, $titlePattern)
        <# Decode HTML entities to ensure proper display of special characters.
         e.g, "Computer Systems: A Programmer&#39;s Perspective"#>
        $title = [System.Net.WebUtility]::HtmlDecode($titleMatches[0].Groups[2].Value)
        if (-not [string]::IsNullOrWhiteSpace($title)) {
            $title = NormalizeTitle $title
        }

        # Handle LCC
        $lccPattern = '(?i)<td class="holding-call-number[^>]*>\s*([^<]*)\s*</td>'
        ...
        # Handle authors
        $authorsPattern = '(?i)<div\s*class="field[^>]*>Title\s*</div><div\s*class="value[^>]*>[^/<]+/\s*([^(<]+)'
        $authorsMatches = [regex]::Matches($bookPageContent, $authorsPattern)
        ...
        if ($authors) {
            $authors = $authors -replace '(based( in part)? on lectures|translated|edited|foreword|introduction|commentary)\s+by\s+[^;]*(;|$)', ''
            $authors = [System.Net.WebUtility]::HtmlDecode($authors)
            $splitPattern = ',| and | & |;'
            $authors = @(
            $authors -split $splitPattern |
                    Where-Object { IsValidAuthor $_ } |
    ...
    return BuildBookData_Yale $bookPageContent
}
...
function InsertLCCManually([string]$targetBook){
    ...
    do {
        ...
        if ($result -ne "" -and -not (IsLcc $result)){
            Write-Host "Enter a valid LCC" -ForegroundColor Yellow
        }
    } while ($result -ne "" -and -not (IsLcc $result))

    return GetLineFromSchedule $result
...
function SelectMostFrequentOrPrompt([string[]]$candidates, [string]$fieldName, [string]$isbn) {
    ...
    $groupedCandidatesByFreq = $candidates |
            Where-Object { -not [string]::IsNullOrEmpty($_) } |
            Group-Object |
            Sort-Object Count -Descending

    # No LCCs or titles
    $hasCandidates = ($candidates | Where-Object { -not [string]::IsNullOrEmpty($_) }).Count -gt 0
    $hasWinner = $null
...
<#
  Searches a text string for ISBN-10 or ISBN-13 patterns and returns the first valid ISBN found ($null otherwise).
  Handles both prefixed (ISBN, ISBN-10, ISBN-13) and standalone formats.
#>
function GetIsbnFromString([string]$text) {
    ...
    $isbnPatterns = @(
        ...
        "(97[89](?:[- \t]?\d){10})",                                                  # Standalone ISBN-13
        "(\d(?:[- \t]?\d){8}(?:[- \t]?[\dX]))"                                        # Standalone ISBN-10
    )
    ...
    foreach ($pattern in $isbnPatterns) {
        ...
        if ($isbnMatches.Count -gt 0) {
            Write-Host "Found possible ISBNs:"
            $isbnMatches | ForEach-Object { Write-Host "- $($_.Groups[1].Value)" }

            foreach ($isbnMatch in $isbnMatches) {
                $isbnCandidate = $isbnMatch.Groups[1].Value

                Write-Host "Testing $isbnCandidate"
                ...
                if (IsIsbn $isbnCandidate) {
                    ...
                    Write-Host "Not a valid ISBN." -ForegroundColor Yellow
                }
            }
        }
    }

    Write-Host "No valid ISBN found." -ForegroundColor Yellow
...
<#
  Attempts to extract the ISBN from a PDF file by scanning its pages in stages. Early pages
  (cover, copyright, introduction, ...) are more likely to contain the correct ISBN, so they are
  prioritized to reduce processing time and minimize false positives.
#>
function GetIsbnFromPdf([string]$pdf) {
    ...
    foreach ($attempt in $extractionAttempts) {
        ...
        if ($content.Trim().Length -gt 0) {
            $isbn = GetIsbnFromString $content
            if ($isbn) {
                break
            }
        }
...
<#
  ...
  Authors are considered the same if:
  1) Their names match when normalized (diacritics removed)
  2) Their last names are identical
  3) their first initials match.
  Returns a list of arrays, each containing names considered variants of the same author.
#>
function GroupBySameAuthor([string[]]$authors) {
    ...
    function SameAuthor([String]$authorA, [String]$authorB){
        ...
        if ($firstAToken[0] -ne $firstBToken[0]) {
            return $false
        }
...
<#
  ...
  - For title: Selects the most frequently returned title across sources, or prompts the user if no clear majority
  - For LCC: Prioritizes Library of Congress. Falls back to secondary sources using frequency-based selection. Prompts if no clear majority.
    - For authors: Collects all returned author names, groups similar variants (e.g., "Doe, John" vs. "John Doe"),
...
function BuildFinalBookData([hashtable]$fetcherToBookDataMap, [string]$isbn) {
    ...
    if ($fetcherToBookDataMap["LibraryOfCongress"].Subject){
        $lcc = $fetcherToBookDataMap["LibraryOfCongress"].Subject
    }
    # Use SearchWorks or CernLibraryCatalogue as secondary sources
        ...
        $lcc = SelectMostFrequentOrPrompt $lccs "LCC" $isbn
    }

    return [BookData]::new($title, $lcc, $authors)
...
function FindBookDataFromSources([string]$isbn) {
    ...
    $fetchers = [Ordered]@{
        ...
        SearchWorks          = "GetData_SearchWorks"
...
function ProcessPDF([string]$pdf) {
    ...
    if (-not $isbn){
        throw "Couldn't process ISBN for $pdf"
    }
    $isIsbn = $true;
    ...
    if (-not $bookData.Title -and
        -not $bookData.Subject -and
        (-not $bookData.Authors -or $bookData.Authors.Count -eq 0)
    ){
        if (Test-Path $inputArg -PathType Container) {
...
function ProcessDirectory([string]$directoryPath) {
    ...
    Write-Host "Processing complete. Output files have been saved to: $desktopPath" -ForegroundColor Green
