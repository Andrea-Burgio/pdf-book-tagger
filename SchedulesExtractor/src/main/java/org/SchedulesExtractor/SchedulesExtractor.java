...
public class SchedulesExtractor {
     ...
     * @param lccToWriter map of LOC subjects to their corresponding BufferedWriter instances
     ...
     */
    private static void processSubfields(XMLStreamReader reader, Map<LOCSubject, BufferedWriter> lccToWriter)
            ...
            if (event == XMLStreamConstants.START_ELEMENT && reader.getLocalName().equals("subfield")) {
                ...
                switch (code) {
                    ...
                    case "z": { //next subfield is going to be 'a'
                        ...
                        bufferedWriter = lccToWriter.get(locSubject); //'z' determines the output file
                    ...
                    case "a": {
                        lastACode = tagContent; // classification code or follows classification code found in 'z'.
                        if (zFound) {   // Previous subfield was 'z'. locSubject is already determined.
                            Objects.requireNonNull(locSubject, "locSubject is null");
                            ...
                            bufferedWriter = lccToWriter.get(locSubject); // 'a' determines the output file
                    ...
                    case "c": {
                        if (lastACode != null && !tagContent.endsWith("Z")) {
                            // skip if it's a pure range (KBM2474 -> KBM2478)
                            buffer.setLength(0);
                            bufferedWriter = null; // don't write this record
                        }
                        else{
                            // merge "a" and "c" -> e.g. QK584.6.A-Z
                            Objects.requireNonNull(locSubject, "locSubject is null");
                            String subjectDescription = locSubject.getDescription();

                            buffer.setLength(0); // clear old content
    ...
    public static void extractSchedules(String XMlClassificationPath) {
        ...
        try (FileInputStream fis = new FileInputStream(XMlClassificationPath);
